...
def compute_error_cell(dirty_df, clean_df, delta):
    ...
    if "SEX" in dirty_df.columns:
        make_col_int(dirty_df, 'ID','RAC1P', 'SEX', 'REGION', 'ST', 'CIT', 'NATIVITY', 'DIS')
    #make_col_string(dirty_df, 'PINCP', 'COW', 'MSP', 'SCHL', 'MIL')
    for i in dirty_df['ID']:
        for j in column_ls:
            if dirty_df.loc[dirty_df['ID'] == i, j].iloc[0] != clean_df.loc[clean_df['ID'] == i, j].iloc[0]:
                cnt +=1
...
def compute_error_tuple(dirty_df, clean_df):
    ...
    cnt = 0
    for i in dirty_df['ID']:
        if not dirty_df.loc[dirty_df['ID'] == i].equals(clean_df.loc[clean_df['ID'] == i]):
            cnt += 1
    return cnt/dirty_df.shape[0]

def display_violation(dir, df,clean, fd_ls, compute_error = True):
    if df.shape[0] > 0:
        for fd in fd_ls:
            fd_filename = dir + f"{fd}.txt"
...
def write_violation(dir, df, clean, fd_ls,fd_dir = 'pums_info/'):
    filename = dir + f"violation.txt"
    with open(filename, 'w') as f:
        if df.shape[0]>0:
            for fd in fd_ls:
                fd_filename = fd_dir + f"{fd}.txt"
                delta = load_fds(fd_filename)
                pairwise_violations = compute_pairwise_violations(df, delta)
                total_pairs = compute_total_pairs(df)
                violated_tuples = compute_violated_tuples(df, delta)
...
def write_violation_matching(dir, df, clean, fd_ls,fd_dir = 'pums_info/'):
    filename = dir + f"violation.txt"
    with open(filename, 'w') as f:
        if df.shape[0]>0:
            for fd in fd_ls:
